...
serve(async (req) => {
  ...
  try {
    ...
    // Call external AI gateway for spam detection
    const AI_GATEWAY_URL = Deno.env.get("AI_GATEWAY_URL") ?? "https://api.ai-gateway.example/v1/chat/completions";
    const AI_GATEWAY_KEY = Deno.env.get("AI_GATEWAY_API_KEY");

    if (!AI_GATEWAY_KEY) {
      throw new Error("AI gateway API key not configured. Set AI_GATEWAY_API_KEY in environment.");
    }
